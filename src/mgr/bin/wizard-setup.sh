...
#//// esta se llama en el setup --> op de setup que la haga. Tb se llama en otros puntos que deberán pasar a ser op.
...
#//// Esta func a root. Allí crear un servicio que reciba los params del wizard y los guarde para ser escritos en el clauer (revisados, claro). luego, leerlos todos del/los ficheros correspondientes en cada invocación a privops (mejor eso o que los busquen uno a uno?).
...
# //// Un programa que corre con sudo, su espacio de memoria es accesible por el usuario que lo ejecuta o sólo por el root? --> parece ser que sólo por el root.
...
#Selector de timezones por defecto (una pequeña comodidad para el usuario)  #//// ROOT
...
case "$LANGUAGE" in 
    ...
    #Ejecutamos las acciones de configuración privilegiadas. (fase 2)
    $PSETUP   2
...
#*-*-Creo que el recovery ya está bien. Regenerar el Cd y probarlo (instalar, reiniciar, backup y recover)
#*-*- Luego ponerme ya con las ops en concreto.
#*-*- Revisar el script de instalación y hacer los cambios de seguridad (permisos extendidos, etc.)
